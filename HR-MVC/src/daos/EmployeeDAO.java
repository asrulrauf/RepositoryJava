...
//    }
    ...
    public List<Employee> getData(Object key, boolean isGetById) {
        ...
        String query = (isGetById)
                ...
                : "SELECT * FROM EMPLOYEES WHERE employee_id LIKE (?) OR first_name LIKE (?) ORDER BY 1";
//        if (isGetById) {
...
//        }
    ...
    public boolean save(Employee E) {
        boolean result = false;
        String query = (getData(E.getId(), true).isEmpty())
                ? "INSERT INTO EMPLOYEES (first_name ,last_name, email, phone_number, hire_date, job_id, salary, commission_pct, manager_id, department_id, employee_id) VALUES ((?),(?),(?),(?),(?),(?),(?),(?),(?),(?),(?))"
